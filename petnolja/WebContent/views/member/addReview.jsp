...
<div id="popup" class="hide">
  <div class="findContent">
    ...
    <div><img src="../../resources/images/temp/petsitter3.jpg" style="height: 80px; float:left; margin-right:20px;"></div>
    <div style="margin-top:15px; margin-bottom: 25px;">
        
        <h6>정성을 다해 사랑으로 돌봐드려요</h6>
        <p>작성자: 김아무개</p>
    </div>
    <hr>
    ...
    <div style="height: 300px; text-align: center;"><h6>어떤점이 좋았나요 ?</h6>
        <div class="form-group">
            <textarea class="form-control" rows="5" id="comment" style="resize:none; height: 180px" placeholder="최소 10자 이상 입력해주세요"></textarea>
            <div style="margin-left:320px; font-size:10pt"><span id="count" >0</span> / 1000<br></div><br>
            <div class="filebox">
                <input class="upload-name" value="" disabled="disabled"> 
                <label for="ex_filename" style="font-size: 11pt;">사진 첨부하기</label> 
                <input type="file" id="ex_filename" class="upload-hidden" onchange="checkSize(this)"> 
            </div>
                <span>            20MB이하의 파일만 첨부 가능합니다.</span><br><br>
                <button type="submit" class="btn btn-primary btn-sm" onclick="alert('변경이 완료되었습니다'); closePopup();">등록</button>&nbsp;
...
<script>
    //팝업 열기
      function openPopup(){$("#popup").removeClass('hide'); 
      $("#starList").children().each(function(){$(this).html('&#9734;');}) // 별초기화
      $("#reviewForm")[0].reset();  //input값 초기화
    }  
       // 팝업 닫기
      function closePopup(){$("#popup").addClass('hide'); 
    }   
      
      // 리뷰 별 바꾸기
      $("#starList").children().on('click',function(){
        var filledStar = $(this).index()+1;  //&#9733;
        for(var i = 0; i< 5;i++){
          if(i<filledStar){
