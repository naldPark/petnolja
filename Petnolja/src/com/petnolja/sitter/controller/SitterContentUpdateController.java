			...
			if(ServletFileUpload.isMultipartContent(request)) {
				...
				for(int i=1; i<=4; i++) {
					...
					if(multiRequest.getOriginalFileName(key) != null) {
						...
						// 그러나
						if(pics[(i*2)-1]!=null) {
							// 기존 파일넘버랑 같은거라면 이미지를 바꾼거기 떄문에 같은 자리에 배치하고
							at.setFileLevel(Integer.parseInt(pics[(i*2)-2])); 
							// 이따  지우러갈꺼니까 arraylist에 넣어놓고
							deleteAttachment.add(pics[(i*2)-1]);
							at.setStatus("N");// STATUS의 의미는 아닌데 새로운이미지인지 구분할려고..
						} else {						
						at.setFileLevel(fileCount);
						at.setStatus("Y"); // STATUS의 의미는 아닌데 새로운이미지인지 구분할려고..
						}
						list.add(at);	
					}
					fileCount++;
